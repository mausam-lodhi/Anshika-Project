			...
			<div className='relative z-10 pt-20 pb-20 px-4 lg:px-24 max-w-7xl mx-auto'>
				...
				<div className='w-full mb-12'>
					<div className='relative group'>
						...
						<img src={imageURL} alt={bookTitle} className='relative rounded-[40px] shadow-2xl w-full h-[400px] lg:h-[550px] object-cover border border-white/10' />
						<div className='absolute bottom-0 left-0 w-full p-8 lg:p-12 bg-gradient-to-t from-black/80 to-transparent rounded-b-[40px]'>
							<h1 className='text-4xl lg:text-6xl font-black leading-tight tracking-tighter mb-2'>{bookTitle}</h1>
							<p className='text-slate-300 text-lg'>
								Resource by <span className='text-blue-400 font-bold'>{authorName || "Shiksha Kendra Scholar"}</span>
							</p>
						</div>
					</div>
				</div>
				...
				<div className='w-full bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[40px] p-8 lg:p-12 mb-16 shadow-2xl'>
					<div className='max-w-5xl'>
						...
						<p className='text-base lg:text-lg text-slate-300 leading-relaxed font-normal opacity-90'>{bookDescription}</p>
							...
							{comments.length > 3 && (
								<button
									...
									{showAllComments ? (
										...
										<>
											<FaChevronDown /> More ({comments.length - 3})
										</>
									)}
								</button>
							)}
